...
import {
  ...
  LanguageService,
  TokenType,
  type TextDocument as LanguageServiceTextDocument,
  type HTMLDocument,
} from 'vscode-html-languageservice'
import { activateAutoInsertion } from './autoInsertion'

function isInsideElixirRegion(
...
function getElixirVirtualContent(
  languageService: LanguageService,
  documentText: string
): string {
...
const toLanguageServiceTextDocument = (
  document: TextDocument
): LanguageServiceTextDocument => ({
  ...document,
...
const toHTMLDocument = (
  ...
  return htmlLanguageService.parseHTMLDocument(
    toLanguageServiceTextDocument(document)
  )
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const getVirtualDocumentUri = (
    document: TextDocument,
    position: Position
  ): Uri => {
    const documentText = document.getText()
    const isElixir = isInsideElixirRegion(
      htmlLanguageService,
      documentText,
      document.offsetAt(position)
    )
    const content = isElixir
      ? getElixirVirtualContent(htmlLanguageService, documentText)
      : documentText
    const ext = isElixir ? 'ex' : 'html'
  ...
  context.subscriptions.push(
    workspace.registerTextDocumentContentProvider('embedded-content', {
      provideTextDocumentContent: (uri) => {
        const originalUri = uri.path
          .slice(1)
          .slice(0, uri.path.lastIndexOf('.') - 1)
  ...
  context.subscriptions.push(
    languages.registerHoverProvider(documentSelector, {
      async provideHover(document, position) {
        const vdocUri = getVirtualDocumentUri(document, position)
  ...
  context.subscriptions.push(
    languages.registerLinkedEditingRangeProvider(documentSelector, {
      async provideLinkedEditingRanges(document, position, _token) {
        const documentText = document.getText()
        const isInsideElixir = isInsideElixirRegion(
          htmlLanguageService,
          documentText,
          document.offsetAt(position)
        )
